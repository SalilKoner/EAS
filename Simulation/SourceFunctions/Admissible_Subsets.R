...
admissible_subsets <- function(Y,X,N=100,steps=15e3,burnin=5e3,PropWeights=NULL,epsilon, 
                               true.Sigma=NULL, addoffset=TRUE, useMCsample4h=FALSE, trueModel, MLasso=TRUE){
  ...
  repeat{
    ...
    if (check.step >= p){
      ...
      return(list("FinalModel"=NULL, "AcceptRatio"=NULL, "ModelProb"=NULL,
                  "Opt.B"=Bhat.opt, "Avg.B"=Bhat.avg, "Opt.model"= integer(0), "Avg.size" = 0,
  ...
  BIC.avg       <- n*as.vector(det.Sighat.avg$modulus) + n*q*(log(2*pi)+1) + {q*mean(mchar.len) + q*(q+1)/2}*log(n)
  BIC.opt       <- n*as.vector(det.Sighat.opt$modulus) + n*q*(log(2*pi)+1) + {q*length(m.opt) + q*(q+1)/2}*log(n)
  
  AcceptRatio = AcceptRatio/steps
  
  list("FinalModel"=m.final, "AcceptRatio"=AcceptRatio, "ModelFreq"=mchar.freq, 
