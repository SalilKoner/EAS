# --------------------------------------------------------
# Complete set of functions to fit MSRL-grp
# --------------------------------------------------------

NN_ADMM_Grp <- function (Y, X, Gamma, Omega, beta, lambda, 
    ...
    while(iterating) {
        ...
        t0 <- betakm1 + H/eta
        t1 <- apply(t0, 1, function(x){sqrt(sum(x^2))})
        betak <- matrix(0, dim(X)[2], dim(Y)[2])
        nonzero.inds <- which(t1 > lam/(rho*eta))
        for (k in nonzero.inds) {
              betak[k,] <- (t1[k] - lam/(rho*eta))*(t0[k,]/t1[k])
        }
        ...
        cond2 <- 1*(s1 < sqrt(p)*tol.abs + tol.rel*sqrt(sum(crossprod(X, Gammak)^2)))
...
MSRL.grp.cv <- function (X, Y, nlambda, lambda.vec = NULL,
        standardize = FALSE, nfolds = NULL, 
    ...
    if (is.null(lambda.vec)) {
        ...
        temp <- svd(y)
        t0 <- crossprod(x, temp$u%*%t(temp$v))
        lambda.max <- 1.01*(1/sqrt(n))*max(apply(t0, 1, function(x){sqrt(sum(x^2))}))
    ...
    for(kk in 1:length(lambda.vec)) {
        ...
        # -------------------------------------------------
        temp <- NN_ADMM_Grp(Y = y, X = x, Gamma = Gamma, Omega = Omega, 
                beta = beta.old, lambda = lambda.vec[kk], 
                tol = tol, maxiter = 1e4, 
                rho =  .0001, eta = 1.00001*xtxeig, 
                quiet=inner.quiet)
    ...
    if (!is.null(nfolds)) {
        ...
        for(k in 1:nfolds) {
            ...
            for(kk in 1:length(lambda.vec)) {
                ...
                temp <- NN_ADMM_Grp(Y = y.inner, X = x.inner, Gamma = Gamma, Omega = Omega, 
                        beta = beta.old, lambda = lambda.vec[kk], 
